...
export default function Home() {
  ...
  function handleQuizGenerated(assessmentId: string) { setGeneratedAssessmentId(assessmentId); setAssessmentRefreshKey((v) => v + 1); }
  ...
  const materialsNode = <div className="productionModule"><DocumentUpload copy={copy} onDocumentProcessed={()=>setMaterialsRefreshKey(v=>v+1)}/></div>;
  ...
  return <>{loadingDashboard && <div className="portalRefreshNotice">{copy.loading}</div>}<PortalExperience locale={locale} localeLabels={localeLabels} fullName={dashboard?.fullName} actualRole={dashboard?.role} competencyScore={dashboard?.competencyScore ?? 0} gaps={dashboard?.gaps ?? []} courses={dashboard?.courses ?? []} profile={dashboard?.profile ?? emptyProfile} onLocaleChange={chooseLocale} onSignOut={handleSignOut} materialsNode={materialsNode} sourceNode={sourceNode} aiNode={aiNode} assessmentNode={assessmentNode} adminNode={isAdmin ? <div className="productionModule"><AdminOverview copy={copy}/></div> : undefined}/></>;
}
