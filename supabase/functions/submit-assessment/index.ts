...
Deno.serve(async (req: Request) => {
  ...
  let correct = 0;
  for (const questionId of questionIds) if (submitted.get(questionId) === keyByQuestion.get(questionId)) correct += 1;
  ...
  if (assessment.competency_id) {
    const gapScore = Math.max(0, 70 - score);
    const priority = gapScore >= 35 ? "HIGH" : gapScore >= 15 ? "MEDIUM" : "LOW";
    const now = new Date().toISOString();
    const { error: competencyError } = await admin.from("user_competencies").upsert({ user_id: userData.user.id, competency_id: assessment.competency_id, score, assessed_at: now }, { onConflict: "user_id,competency_id" });
    if (competencyError) return new Response(JSON.stringify({ error: "competency_update_failed" }), { status: 500, headers: jsonHeaders });

    const { error: gapError } = await admin.from("skill_gaps").upsert({ user_id: userData.user.id, competency_id: assessment.competency_id, priority, gap_score: gapScore, rationale: gapScore > 0 ? "Latest assessment score is below the target competency level." : "Target competency level is met.", created_at: now }, { onConflict: "user_id,competency_id" });
