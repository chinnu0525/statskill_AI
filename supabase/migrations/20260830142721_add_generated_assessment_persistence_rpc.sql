...
begin
  ...
  insert into public.assessments (title, competency_id, locale, owner_id, source_document_id, origin)
  values (trim(p_title), null, p_locale, p_owner_id, p_document_id, 'AI_GENERATED')
...
revoke all on function public.persist_generated_assessment(uuid, uuid, text, text, jsonb) from public, anon, authenticated;
grant execute on function public.persist_generated_assessment(uuid, uuid, text, text, jsonb) to service_role;
