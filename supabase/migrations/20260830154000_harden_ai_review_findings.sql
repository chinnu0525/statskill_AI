...
as $$
  with search_query as (
    select websearch_to_tsquery('simple', coalesce(trim(p_query), '')) as query
  )
  select
    c.id,
    c.document_id,
    ...
    ts_rank(c.search_vector, q.query)::real as rank
  from public.document_chunks c
...
revoke all on function public.search_user_document_chunks(uuid, text, integer) from public, anon, authenticated;
grant execute on function public.search_user_document_chunks(uuid, text, integer) to service_role;
